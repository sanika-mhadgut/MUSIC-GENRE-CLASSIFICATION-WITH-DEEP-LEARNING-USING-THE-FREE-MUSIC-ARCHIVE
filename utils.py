...
#             self.Y = np.empty((self.batch_size, Y.shape[1]), dtype=np.int64)
...
class SampleLoader:
    def __init__(self, audio_dir, Y, tids, loader, batch_size=4):
        ...
        self.tids = np.ctypeslib.as_array(data)
        self.X = np.empty((self.batch_size, *loader.shape))
        self.Y_data = np.empty((self.batch_size, Y.shape[1]), dtype=np.int64)
    ...
    def __next__(self):
        ...
        for tid in tids:
            try:
                audio_path = get_audio_path(self.audio_dir, tid)
                self.X[actual_batch_size] = self.loader.load(audio_path)
                self.Y_data[actual_batch_size] = self.Y.loc[tid]
                actual_batch_size += 1
            except Exception as e:
                print("\nIgnoring " + audio_path + " (error: " + str(e) + ").")
        ...
        with self.lock2:
            ...
            return self.X[:actual_batch_size], self.Y_data[:actual_batch_size]
            
def build_sample_loader(audio_dir, Y, tids, loader, batch_size=4):
    return SampleLoader(audio_dir, Y, tids, loader, batch_size)
